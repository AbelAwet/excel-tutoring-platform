...
export const createReview = async (req, res) => {
  try {
    ...
    if (tutor) {
      await sendReviewNotification(tutor.user._id, req.user._id, `${req.user.firstName} ${req.user.lastName}`, rating);
    }
...
export const getTutorReviews = async (req, res) => {
  try {
    ...
    const reviews = await Review.find({ tutor: req.params.tutorId, isPublished: true })
      .populate('student', 'firstName lastName avatar')
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(parseInt(limit));

    const total = await Review.countDocuments({ tutor: req.params.tutorId, isPublished: true });

    const ratingDistribution = await Review.aggregate([
      { $match: { tutor: new mongoose.Types.ObjectId(req.params.tutorId), isPublished: true } },
    ...
    res.status(200).json({
      success: true,
      data: { reviews, ratingDistribution, pagination: { page: parseInt(page), limit: parseInt(limit), total, pages: Math.ceil(total / limit) } }
    });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message || 'Failed to get reviews' });
  }
};
...
export const respondToReview = async (req, res) => {
  try {
    ...
    if (!tutor) return res.status(403).json({ success: false, message: 'Not authorized' });

    review.tutorResponse = { comment, respondedAt: new Date() };
    await review.save();

    res.status(200).json({ success: true, message: 'Response added successfully', data: { review } });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message || 'Failed to respond to review' });
  }
};
...
export const reportReview = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: error.message || 'Failed to report review' });
  }
};
